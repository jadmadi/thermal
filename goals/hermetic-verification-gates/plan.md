...
- [ ] **Task 3 — Add user-level semantic assertions**: Assert static/web/live token parity, unknown receipt evidence, partial coverage and database cache isolation after their owning goals land. Include real server start/stop and SSE cancellation smoke coverage.
...
Start after `receipt-accounting-coverage`, `live-metric-parity`, `devin-cache-source-isolation`, `localhost-request-boundary`. Read prerequisite contracts before editing shared code.
